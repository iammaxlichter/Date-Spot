    ...
    const { error: profileErr } = await supabase.from("profiles").upsert(
      {
        id: data.user.id,
        name,
        // if profiles has an email column and your trigger handles it, don't force it
      },
      { onConflict: "id" }
    );

    if (profileErr) {
      // Non-fatal in most setups; but you probably want to know.
      // Throwing keeps you honest during setup.
      throw profileErr;
    }
  }
